...
class TransformerAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent_args.add_argument("--max_history", type=int, default=7, help="Number of previous utterances to keep in history")
    ...
    def __init__(self, opt, shared=None):
        ...
        if shared is None:
            ...
            self.model_checkpoint = TmemaModel.from_pretrained(args.model_checkpoint)
    ...
    def act(self):
        if self.args.eval_type == "hits@1" and len(self.candidates) > 0:
            ...
            #print('\n'.join(self.tokenizer.batch_decode(self.persona)))
            #print('\n'.join(self.tokenizer.batch_decode(self.history)))
            print(ans)
            #print('-'*50)
            '''
            with open('./wo_EM.tsv', 'a') as f:
                tsv_w = csv.writer(f, delimiter='\t')
                #tsv_w.writerow(['id', 'name', 'score'])  
                tsv_w.writerow(['\n'.join(self.tokenizer.batch_decode(self.persona)), '\n'.join(self.tokenizer.batch_decode(self.history)),ans])
            '''
        return reply
