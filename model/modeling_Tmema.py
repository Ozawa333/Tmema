...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        self.g_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        
        #self.fc = nn.Linear(2 * self.head_dim, self.head_dim, bias=bias)
    ...
    def forward(
        ...
        given: Optional[torch.Tensor] = None,
        ...
        if given is not None:
            given_states = self.g_proj(given) * self.scaling
            query_states = query_states + given_states
        '''
        # get key, value proj
        if is_cross_attention and past_key_value is not None:
            #print("reuse k,v, cross_attentions")
            ...
            #print("reuse k, v, self_attention")
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
            if given is not None:
                given_states = self._shape(self.g_proj(given), -1, bsz)
                value_states = value_states + given_states
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if latent_memory != None:
            assert latent_memory.size() == inputs_embeds.size()
            #hidden_states = hidden_states + latent_memory
            #print(hidden_states)
            #print(latent_memory)
            #print(latent_memory.shape, '-'*50)
...
class TmemaModel(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig, num_token=None, num_latent=40000, num_latent2=40000):
        super().__init__(config)
        self.model = BartModel(config)
    ...
    def forward(
        ...
        if infer:
            if infer_lmlabels is not None:
                ...
                infer_latent_hidden_state = infer_encoder_outputs.last_hidden_state
                infer_latent_logits = self.latent_head_m1(infer_latent_hidden_state)

                weight_memory = torch.matmul(torch.softmax(infer_latent_logits, dim=-1), self.memory1)

                weight_memory_select = torch.full([len(infer_decoder_input_ids), 
                                            len(infer_decoder_input_ids[0]), 
                                            self.config.d_model], 0).to(torch.float32).to(self.device)
                
                infer_decoder_input_ids_list = infer_decoder_input_ids.tolist()
                infer_input_ids_list = infer_input_ids.tolist()
                
                for batch in range(len(infer_decoder_input_ids_list)):
                    for i, token_id in enumerate(infer_decoder_input_ids_list[batch]):
                        if token_id in infer_input_ids_list[batch] and token_id < self.num_latent:
                            weight_memory_select[batch][i] = weight_memory[batch][infer_input_ids_list[batch].index(token_id)]                            
                ...
                infer_decoder_outputs = self.model.decoder(
                    ...
                    output_hidden_states=output_hidden_states,
                    return_dict=return_dict,
                    latent_memory=weight_memory_select
                )
        ...
        if input_ids is not None:
            self.input_ids_recorder = input_ids
            if infer:
                ...
                latent_hidden_state_m1 = encoder_per_outputs.last_hidden_state
                latent_logits_m1 = self.latent_head_m1(latent_hidden_state_m1)
                entail_latent_variable = torch.matmul(torch.softmax(latent_logits_m1, dim=-1), self.memory1)
            ...
            latent_hidden_state_m2 = encoder_outputs.last_hidden_state
            if dialog:
                latent_logits_m2 = self.latent_head_m2(latent_hidden_state_m2)
                dialog_latent_variable = torch.matmul(torch.softmax(latent_logits_m2, dim=-1), self.memory2)

            if generate:
                if infer and dialog:
                    return {"latent_variable": dialog_latent_variable + torch.nn.functional.pad(entail_latent_variable,(0,0,0,len(dialog_latent_variable[0])-len(entail_latent_variable[0])),value=0),
                            "encoder_outputs": encoder_outputs}
                elif not dialog:
        ...
        if decoder_input_ids is not None:
            if latent_variable is not None:
                self.input_latent = latent_variable
            elif dialog_latent_variable is not None and entail_latent_variable is not None:
                self.input_latent = dialog_latent_variable + torch.nn.functional.pad(entail_latent_variable,(0,0,0,len(dialog_latent_variable[0])-len(entail_latent_variable[0])),value=0)
            elif entail_latent_variable is not None and dialog_latent_variable is None:
                self.input_latent = entail_latent_variable
            elif dialog_latent_variable is not None and entail_latent_variable is None:
                self.input_latent = dialog_latent_variable
            #else:
                #input_latent = None

            input_latent_select = torch.full([len(decoder_input_ids), 
                                                 len(decoder_input_ids[0]), 
                                                 self.config.d_model], 0).to(torch.float32).to(self.device)

            if len(input_latent_select) != len(self.input_latent):
                self.input_latent = self.input_latent.repeat(len(input_latent_select),1,1)
            
            if len(input_latent_select) != len(self.input_ids_recorder):
                self.input_ids_recorder = self.input_ids_recorder.repeat(len(input_latent_select),1)
                
            decoder_input_ids_list = decoder_input_ids.tolist()
            input_ids_list = self.input_ids_recorder.tolist()
            for batch in range(len(decoder_input_ids_list)):
                for i, token_id in enumerate(decoder_input_ids_list[batch]):
                    if token_id in input_ids_list[batch] and token_id < self.num_latent:
                    #if token_id in input_ids_list[batch] and token_id < 10000:
                        input_latent_select[batch][i] = self.input_latent[batch][input_ids_list[batch].index(token_id)]
            ...
            decoder_outputs = self.model.decoder(
                    ...
                    output_hidden_states=output_hidden_states,
                    return_dict=return_dict,
                    latent_memory=input_latent_select,
                )
            ...
            bow_logits = self.bow_head(input_latent_select).contiguous() #for notdialog
